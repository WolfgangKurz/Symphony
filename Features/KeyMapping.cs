...
namespace Symphony.Features {
	[Feature("KeyMapping")]
	internal class KeyMappingFeature : MonoBehaviour {
		public void Start() {
			KeyMappingConf.Load();
		}
		...
		public void Update() {
			if (Conf.KeyMapping.Use_KeyMapping.Value) {
				try {
					var act = Conf.KeyMapping.Activated.Value;
		...
		public void OnGUI() {
			if (UIManager.Instance?.GetPanel<KeyMapPanel>() == null && Conf.KeyMapping.Use_KeyMapping.Value) {
				var KeyMap_Alpha = Conf.KeyMapping.Opacity.Value;
				if (KeyMap_Alpha > 0f) {
					var act = Conf.KeyMapping.Activated.Value;
					if (KeyMappingConf.KeyMaps.ContainsKey(act)) {
						var maps = KeyMappingConf.KeyMaps[act];
